...
export const getDashboardStats = TryCatch(async (req, res, next) => {
  ...
  if (myCache.has(key)) stats = JSON.parse(myCache.get(key) as string);
    ...
    const categoryCount = await getInventories({
        categories: categories.filter((category): category is string => !!category),
...
export const getPieCharts = TryCatch(async (req, res, next) => {
  ...
  if (myCache.has(key)) charts = JSON.parse(myCache.get(key) as string);
    ...
    const discount = allDonations.reduce(
      (prev, Donation) => prev + (Donation.reward || 0),
      0
    );

    const CampaignionCost = allDonations.reduce(
      (prev, Donation) => prev + (Donation.tax || 0),
      0
    );
...
export const getBarCharts = TryCatch(async (req, res, next) => {
  ...
  if (myCache.has(key)) charts = JSON.parse(myCache.get(key) as string);
    ...
    const typedCampaigns = Campaigns.map(doc => doc.toObject()) as MyDocument[];
    const typedUsers = users.map(doc => doc.toObject()) as MyDocument[];
    const typedDonations = Donations.map(doc => doc.toObject()) as MyDocument[];

    const CampaignCounts = getChartData({ length: 6, today, docArr: typedCampaigns });
    const usersCounts = getChartData({ length: 6, today, docArr: typedUsers });
    const DonationsCounts = getChartData({ length: 12, today, docArr: typedDonations });
